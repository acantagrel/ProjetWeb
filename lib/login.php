    ...
    <body>
        ...
        <?php
            if (!empty($_POST['login']) || !empty($_POST['mdp']))
            {
                $login=$_POST['login'];
                $mdp=$_POST['mdp'];
                $req1=$BDD->prepare("SELECT * FROM USER WHERE LOGIN=?");//si le login existe
                $req1->execute(array($_POST['login']));
                $ligne1=$req1->fetch();

                $req2=$BDD->prepare("SELECT * FROM USER WHERE LOGIN=? AND MDP=?");//si le mdp correspond au login entré
                ...
                {
                    ?> <button type="button" class="btn btn-warning btn-lg" >Mot de passe incorrect</button> <?php
                }
                else if ($ligne1==NULL && $login !="" )
                {
                    ?><button type="button" class="btn btn-warning btn-lg" >Cet utilisateur est inconnu</button><?php
                }
                else if ($ligne2!=NULL){
                    //print "Bienvenue ".escape($_POST['login'])." !";
                    //print "<br/> <br/> <a href='../index.php' ><input type='button' value='Accueil'></a>";
